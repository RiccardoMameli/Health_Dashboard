...
| D3 | Wire the remaining live tiles | 1–2 | **Mostly done 24 Sep 2026.** Sleep, resting HR, weight, Steps and Sleep trend render live data with their real source; the check-in card is live. The supplements card ticks for real (same day). Left: energy balance (blocked on O2) and last-sync times from `/sync/health` |
